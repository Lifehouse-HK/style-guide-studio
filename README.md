...
The **desktop editor** now has a compact Office-style ribbon, a single structured writing canvas, an outline, optional bilingual alignment, shared tables, manual provision insertion and clickable number editing at every numbered level, local/external reference selection, defined names, figures, guided amendment operations including whole Parts, project/AKN files, recovery and on-demand common-engine HTML preview. Run `npm run dev` or build the fully static app with `npm run build:editor`. Document canvas text and web output prefer Times New Roman; the interface uses sans-serif; interface icons use a locally bundled icon font. PDF typography remains a separate provisional publication setting. The real document collection and deployed publication website remain a separate, deferred repository. This is not yet a release-ready product; remaining viewer, browser and integration acceptance gates are recorded in the plan.
