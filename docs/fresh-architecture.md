...
Added 2026-09-12. A `definitions` content block contains an ID, a `master` flag and stable-ID items with paired `term`, restricted inline-HTML `meaning` and optional paired `orderBy`. It is not a numbered provision level and cannot give a subsection a heading. Lists can sit directly in sections or in subsections and other prose-capable provisions. Introductory scope wording remains an ordinary text block. Meanings include their relationship word (means/includes/etc.); quotes, indentation and terminal semicolon/full stop are generated. Entries do not yet contain nested structural paragraphs or tables.
