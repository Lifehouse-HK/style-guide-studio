...
Added 2026-09-12. A `definitions` content block contains an ID, a `master` flag and stable-ID items with paired `term`, restricted inline-HTML `meaning` and optional paired `orderBy`. It is not a numbered provision level and cannot give a subsection a heading. Lists can sit directly in sections or in subsections and other prose-capable provisions. Introductory scope wording remains an ordinary text block. Meanings include their relationship word (means/includes/etc.); quotes, indentation and terminal semicolon/full stop are generated. Entries may contain structured numbered branches, a shared table and closing text.

Local lists do not register aliases or substitute ordinary prose. The existing References aliases remain independent. At most one active master list may exist across the Guide. It merges local items with all aliases at rendering/editing time, so no duplicate editable copy is stored. References saves the target's paired formal titles and optional ordering keys in optional `aliasDetails`; these snapshots make generated definitions stable offline. Catalogues supply HTML/PDF links and serve as a display fallback for older aliases, but Checks asks authors to save missing title snapshots before enacting a master list. Removing or changing the master flag does not delete the separate aliases.

Lexical ordering is deterministic and locale-independent: NFC-normalise and lowercase the effective key, compare lexically, then use stable ID to break ties. Remove an exact lowercase initial `the` plus whitespace from the term only. Do not remove capitalised `The`, or an article from an explicit ordering key. A nonblank language-specific override replaces the automatic key. Chinese defaults to Unicode lexical ordering; author keys can prescribe another sequence. Separate expressions sort by their own language. Parallel output sorts by English and aligns the same bilingual entries; no independent column sorting. Source arrays are not reordered, so editing, identities and amendment preconditions remain stable.

Checks report incomplete paired terms/meanings, missing master alias/title data, duplicate terms within each list (including automatic names) and duplicate item IDs. JSON and the embedded XML source preserve all fields; the XML projection emits unnumbered blockList/item/def elements. Definitions have stable public anchors and individual insertion/substitution/repeal operations. Whole-provision substitution retains omitted definition/branch identities as tombstones. Master document-name actions update the principal alias registry; see the amendment authoring section below.

## Complete amendment authoring follow-up

The audit completion programme adds definition-level insertion/substitution/repeal, master document-name insertion/substitution/repeal, anchored word insertion/omission and formal-title/long-title/preamble substitution. Each operates on the checked intermediate source, generates bilingual operative wording and preserves the original. The principal enacting formula remains excluded. Omitted definitions and their child paragraphs retain tombstones and reference destinations. Definition child numbering/parent identity cannot be altered through substitution.

Definitions can contain manually labelled paragraphs and subordinate paragraphs, an ordinary shared table and closing text. Their numbering has a separate scope per definition. Their typed structure participates in references, diagnostics, rendering and XML, rather than being disguised as text.

Amendment instruments may also contain structured supplemental sections and Schedules for standalone transitional/savings material. These are authored in a separate workspace and belong to the amendment, never automatically to the principal. Actual changes to the principal still require actions. Enactment validates the combined generated/supplemental numbering, languages, references and front matter.
