...
export async function render(
  ...
  const esc = escape,
    fmt = (s: string, l: Language) => inline(s, g, l, options.catalogues, options.pdf),
  ...
  const pre =
    document.preamble.mode === 'paragraph'
      ? paired((l) => `<p>${fmt(document.preamble.paragraph[l], l)}</p>`)
      : document.preamble.mode === 'list'
        ? document.preamble.items
            .map((p, i) =>
              paired((l) => `<p><span class="number">${i + 1}.</span>${fmt(p[l], l)}</p>`),
            )
            .join('')
        : '';
  return `<!doctype html><html lang="${langs[0] === 'zh' ? 'zh-Hant' : 'en'}"><head><meta charset="utf-8">${options.iframe ? '<base href="about:srcdoc">' : ''}<title>${esc(document.titles[langs[0]])}</title><style>${stylesheet}${layout === 'parallel' ? '@page{size:A4 landscape}' : ''}</style></head><body class="${layout}">${paired((l) => `<h1>${esc(document.titles[l])}</h1>`)}<div class="status">${status}</div>${toc}${paired((l) => `<p>${fmt(document.longTitle[l], l)}</p>`)}${pre}${paired((l) => `<p>${fmt(document.formula[l], l)}</p>`)}${body}${options.history && options.revision?.history.length ? `<aside class="history"><h2>Amendment history / 修訂紀錄</h2>${options.revision.history.map((h) => `<p>${esc(h.date)} — ${esc(h.titles[langs[0]])}, ${esc(h.clause)} — ${esc(h.action)}</p>`).join('')}</aside>` : ''}</body></html>`;
}
